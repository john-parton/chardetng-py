...
@click.option(
    ...
    "-k",
    type=int,
    help="Number of files to read into memory for benchmarks.",
    default=200,
)
@click.option(
    "--iterations",
    "-n",
    type=int,
    help="Number of iterations of timeit to run.",
    default=12,
)
@coro
async def main(*, input_directory: Path, sample_size: int, iterations: int):
    ...
    for test_function in test_functions:
        total_time = Timer(
            "test_func(data)", globals={"test_func": test_function, "data": data}
        ).timeit(number=iterations)
